...
const SelectorEspecialidade = ({ onSelectionChange, defaultValue }) => {
    ...
    const [selectEspecialidade, setSelectEspecialidade] = useState({ value: defaultValue.value, label: defaultValue.label } || { value: '', label: 'Selecione uma especialidade' });
    ...
    useEffect(() => {
        if (defaultValue) {
            const foundOption = opcoesEspecialidades.find(option => option.value === defaultValue.value);
            if (foundOption) {
                setSelectEspecialidade(foundOption);
            }
        }
    }, [defaultValue, opcoesEspecialidades]);
